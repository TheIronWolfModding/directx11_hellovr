...
CameraClass *m_CameraLeft = nullptr, *m_CameraRight = nullptr;
ModelClass* m_Model = nullptr;
ModelClass* m_hamLeftModel = nullptr;
ModelClass* m_hamRightModel = nullptr;
bool m_hamAvailable = false;
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
  ...
  Matrix4 matrixObj(
  0.984807789f, 0.0f, -0.173648164f, 0.0, 0.0f, 1.0f, 0.0f, 0.0, 0.173648164, 0.0f, 0.984807789f, 0.0, -0.0299908444f, 0.0f, 0.0f, 1.0f);
...
void MyDebug(LPCWSTR msg) { MessageBox(NULL, msg, _T(""), 0); }
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
  ...
  szWindowClass, szTitle, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0, clientWidth, clientHeight, nullptr, nullptr, hInstance, nullptr);
...
Matrix4 GetHMDMatrixPoseEye(vr::Hmd_Eye nEye)
{
  ...
  outFile << "Eye " << (nEye == vr::Hmd_Eye::Eye_Left ? "Left=" : "Right=") << std::endl
          << MatrixToString(matrixObj).c_str() << std::endl;
  matrixObj.invert();
...
void SetupCameras()
{
  ...
  outFile << "Proj Right=" << std::endl
          << MatrixToString(m_mat4ProjectionRight).c_str() << std::endl;
  outFile << "Eye Left Inverted=" << std::endl
          << MatrixToString(m_mat4eyePosLeft).c_str() << std::endl;
  outFile << "Eye Right Inverted=" << std::endl
          << MatrixToString(m_mat4eyePosRight).c_str() << std::endl;
...
bool init(HWND hWnd)
{
  ...
  if (eError != vr::VRInitError_None) {
    ...
    char buf[1024];
    sprintf_s(
    buf, ARRAYSIZE(buf), "Unable to init VR runtime: %s", vr::VR_GetVRInitErrorAsEnglishDescription(eError));
    string temp(buf);
  ...
  D3D_DRIVER_TYPE driverTypes[] = { D3D_DRIVER_TYPE_HARDWARE, // the first thing to try, if failed, go to the next
                                    D3D_DRIVER_TYPE_WARP,
                                    D3D_DRIVER_TYPE_REFERENCE };
  UINT numDriverTypes = ARRAYSIZE(driverTypes);

  D3D_FEATURE_LEVEL featureLevels[] = { D3D_FEATURE_LEVEL_11_0, // texture size and others..
  ...
  for (unsigned i = 0; i < numDriverTypes; ++i) {
    errorCode = D3D11CreateDeviceAndSwapChain(NULL,
                                              driverTypes[i],
                                              NULL,
                                              createDeviceFlags,
                                              featureLevels,
  ...
  if (hamLeft.pVertexData == nullptr) {
    m_hamAvailable = false;
    return true; // Done
  }
  ...
  for (int tri = 0; tri < hamLeft.unTriangleCount; ++tri) {
    ...
    str << L"{" << (hamLeft.pVertexData + (tri * 3))->v[0] << L","
        << (hamLeft.pVertexData + (tri * 3))->v[1] << L"},\n";
    str << L"{" << (hamLeft.pVertexData + (tri * 3) + 1)->v[0] << L","
        << (hamLeft.pVertexData + (tri * 3) + 1)->v[1] << L"},\n";
    str << L"{" << (hamLeft.pVertexData + (tri * 3) + 2)->v[0] << L","
        << (hamLeft.pVertexData + (tri * 3) + 2)->v[1] << L"},\n";
    ;
  ...
  for (int tri = 0; tri < hamRight.unTriangleCount; ++tri) {
    std::wstringstream str;
    str << (hamRight.pVertexData + (tri * 3))->v[0] << L"," << (hamRight.pVertexData + (tri * 3))->v[1]
        << L"," << (hamRight.pVertexData + (tri * 3) + 1)->v[0] << L","
        << (hamRight.pVertexData + (tri * 3) + 1)->v[1] << L","
        << (hamRight.pVertexData + (tri * 3) + 2)->v[0] << L","
        << (hamRight.pVertexData + (tri * 3) + 2)->v[1] << L"\n";
...
bool RenderScene(vr::Hmd_Eye nEye)
{
  ...
  if (m_hamAvailable) {
    ...
    result = m_hamColorShader->Render(
    pImmediateContext, hamModel->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, hamModel->GetTexture());
  ...
  pImmediateContext, m_Model->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, m_Model->GetTexture());
  if (!errorshown && !result) {
    errorshown = true;
    return false;
...
void render_frame(void)
{
  ...
  result = m_ColorShader->Render(pImmediateContext,
                                 m_DebugWindowLeft->GetIndexCount(),
                                 identity,
                                 identity,
                                 orthoMatrix,
                                 m_RenderTextureLeft->GetShaderResourceView());
  if (!result) {
    return;
  }
  ...
  // Render the debug window using the texture shader.
  result = m_ColorShader->Render(pImmediateContext,
                                 m_DebugWindowRight->GetIndexCount(),
                                 identity,
                                 identity,
                                 orthoMatrix,
                                 m_RenderTextureRight->GetShaderResourceView());
  if (!result) {
    return;
  }
  ...
  = { m_RenderTextureLeft->GetTexture(), vr::TextureType_DirectX, vr::ColorSpace_Auto };
  vr::EVRCompositorError error1 = vr::VRCompositor()->Submit(vr::Eye_Left, &leftEyeTexture);
  vr::Texture_t rightEyeTexture
  = { m_RenderTextureRight->GetTexture(), vr::TextureType_DirectX, vr::ColorSpace_Auto };
