...
bool ColorShaderClass::Initialize2(ID3D11Device* device, HWND hwnd)
{
	bool result;


	// Initialize the vertex and pixel shaders.
	result = InitializeShader2(device, hwnd, L"hamTexture.vs", L"hamTexture.ps");
	//result = InitializeShader(device, hwnd, L"shaders.shader", L"shaders.shader");
	if(!result)
	{
		return false;
...
bool ColorShaderClass::InitializeShader2(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	ID3D10Blob* errorMessage;
	ID3D10Blob* vertexShaderBuffer;
